...
class StockApp:
    ...
    def _build_table(self):
        ...
        # 分隔线样式
        self.tree.tag_configure("sep", background="#1565C0")  # 深蓝色竖分隔线
        self.tree.tag_configure("hline", background="#1565C0")  # 深蓝色横分隔线
    ...
    def _refresh_table(self):
        ...
        for idx, (rid, item) in enumerate(items):
    ...
    def search_item(self):
        ...
        for row in self.tree.get_children(): self.tree.delete(row)
        # 收集匹配项
        matched = [(rid, item) for rid, item in self.data.items()
                   if keyword in item.get("model","").lower() or keyword in item.get("info","").lower()]
        for i, (rid, item) in enumerate(matched):
            ...
            if i < len(matched) - 1:
                self.tree.insert("", "end",
                                 values=("", "", "", "", "", "", "", "", ""),
                                 tags=("hline",))
        self.count_label.config(text=f"搜索到 {len(matched)} 条结果" if keyword else "")
